<x-app-layout>
    <x-slot name="header">
        ...
        {!! __('Transaction &raquo; ') !!} {{$transaction->food->name}} by {{$transaction->user->name}}
        </h2>
    </x-slot>
              ...
              <div class="w-full md:w-5/6 px-4 mb-4 md:mb-0">
                <div class="flex flex-wrap mb-3">
                  ...
                  <div class="w-1/6">
                    ...
                    <div class="text-xl font-bold">{{ number_format($transaction->qty) }}</div>
                  </div>
                  <div class="w-1/6">
                ...
                <div class="flex flex-wrap mb-3">
                  <div class="w-5/6">
                    ...
                    <div class="text-lg">{{ $transaction->paymentUrl }}</div>
                  </div>
                  <div class="w-1/6">
                    <div class="text-sm mb-1">Change Status</div>
                    <a href="{{ route('transactions.changeStatus', ['id' => $transaction->id, 'status' => 'ON_DELIVERY']) }}" class="bg-blue-500 hove:bg-blue-700 text-white font-bold px-2 rounded block text-center w-full mb-1">On Delivery</a>
                    <a href="{{ route('transactions.changeStatus', ['id' => $transaction->id, 'status' => 'DELIVERED']) }}" class="bg-green-500 hove:bg-green-700 text-white font-bold px-2 rounded block text-center w-full mb-1">Delivered</a>
                    <a href="{{ route('transactions.changeStatus', ['id' => $transaction->id, 'status' => 'CANCELLED']) }}" class="bg-red-500 hove:bg-red-700 text-white font-bold px-2 rounded block text-center w-full mb-1">Cancelled</a>
                  </div>
                </div>
              </div>
             </div> 
